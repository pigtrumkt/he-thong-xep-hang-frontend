...
export default function ServiceReportPage() {
  ...
  async function openComments(service: any) {
    setSelectedService(service);
    setComments([]);
    setShowComments(true);

    const res = await apiGet(
      `/tickets/serviceComments?serviceId=${service.id}&from=${fromDate}&to=${toDate}`
    );
          ...
          {loading ? (
            ...
            data.map((svc, idx) => (
              ...
              >
                ...
                <td className="px-4 py-3">{svc.service_name}</td>
